...
const server_url='http://admin.cureassist.co/api';
...
class App extends Component {
  ...
  handlePayment(){
      ...
      let doctor_id = this.state.selectedDoctor.value;

      fetch(server_url+`/doctor/book/slot?id=${encodeURIComponent(slotid)}&appointment_date=${encodeURIComponent(date)}&doctor_id=${encodeURIComponent(doctor_id)}`,{
          method:"POST"
        ...
        .then(
          (result) => {
              ...
              fetch(server_url+`/doctor/slot/status/update?appointment_id=${encodeURIComponent(bookingid)}&payment_id=${encodeURIComponent(paymentid)}&status=${encodeURIComponent(status)}`,{method:"POST"})
              .then(response => response.json())
              .then(
                (result) => {
  ...
  componentDidMount(){
    ...
    fetchSlots(){
      ...
      fetch(server_url+`/doctor/slot?appointment_date=${appointment_date}&doctor_id=${encodeURIComponent(doctor_id)}`)
      .then(res => res.json())
      .then(
        (result) => {
